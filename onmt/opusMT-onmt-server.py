    ...
    def handleMessage(self):
        ...
        for s in sentSource:
            ...
            if key in cache:
                ...
                ## TODO: it's not always good to remove the fist word
                ## --> this is specific to the multilingual AmericasNLP model
                ##     that spits out target language labels also in translation
                received_tgt = received[0][0]['tgt'].split(' ', 1)[1]

                ## undo segmentation
                if args.bpe:
                    translated = received_tgt.replace('@@ ','')
                elif args.spm:
                ...
                cache[key] = detokenized + "\t" + segmented + "\t" + received_tgt + "\t" + alignment
